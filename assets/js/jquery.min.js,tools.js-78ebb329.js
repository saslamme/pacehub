...
(function () {
  ...
  function normPaceStr(str){
    ...
    // allow mm:ss or m:ss or just mm
    var parts = s.split(':');
    if (parts.length === 1) {
    ...
    if (!isFinite(mm) || mm<0) mm = 0;
    if (!isFinite(ss) || ss<0) ss = 0;
  ...
  function bindMany(el, events, fn){
    if (!el) return;
    events.split(' ').forEach(function(ev){ el.addEventListener(ev, fn, false); });
  }
