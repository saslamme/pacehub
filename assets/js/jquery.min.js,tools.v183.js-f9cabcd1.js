...
(function () {
  ...
  function attachPace(root) {
    ...
    function recalc(){
      ...
      // Sync inputs if missing
      if (pace && (!haveP || lastChanged==='speed') && isFinite(paceS)) {
        var pOut = (unit==='mi') ? paceS/1.609344 : paceS; pace.value = secToMMSS(pOut);
      }
      if (speed && (!haveV || lastChanged==='pace') && isFinite(Vkmh)) {
        var vOut = (unit==='mi') ? (Vkmh/1.609344) : Vkmh; speed.value = vOut.toFixed(2); if (speedSlider) speedSlider.value = speed.value;
      }
