                ...
                var t = function() {
                    function i(t, e) {
                        ...
                        this.options.enableKeyboard && this.addEventListener(document.body, "keyup." + this.eventNamespace, this.throttle(function(t) {
                            if (n.controlCoordinates.swipeDiff = 0,
                            n.isAnimating && "Escape" === t.key)
                                return n.currentImage.setAttribute("src", ""),
                                n.isAnimating = !1,
                                n.close();
                    ...
                    (e = [{
                        ...
                        value: function(t) {
                            ...
                            if (s += "" !== o ? "#".concat(o) : "",
                            n)
                                for (var a = n.split(/\s/), r = 0; r < a.length; r++)
                                    s += ".".concat(a[r]);
                        ...
                        value: function(t) {
                            ...
                            (document.body.clientWidth < t || this.isAppleDevice) && (e = parseInt(document.body.style.paddingRight || 0, 10),
                        ...
                        value: function(a, t, r, e) {
                            var o, l = this, n = m(a = this.wrap(a));
                            try {
                                for (n.s(); !(o = n.n()).done; ) {
                                    var i = o.value;
                                    i.style.opacity = 0,
...
window.addEventListener('load', () => {
    ...
    ( () => {
        ...
        function setHeaderScrollProgress() {

            const maxDepth = 200;
            const scrollDepth = Math.min(window.scrollY, maxDepth);
            const value = (scrollDepth / maxDepth);

            document.documentElement.style.setProperty('--header-scroll-progress', value);
        }

        window.addEventListener('scroll', setHeaderScrollProgress);
    ...
    ( () => {
        ...
        const initLightbox = function() {
            ...
            const lighboxGroups = new Set(Array.from(document.querySelectorAll('a[data-lightbox]:not([disabled])')).map(a => a.dataset.lightbox));

            lighboxGroups.forEach(group => {

                new SimpleLightbox(`a[data-lightbox="${group}"]`,{
                    navText: ['', ''],
                    captionSelector: (el) => {
                        return el.parentNode.querySelector('figcaption');
                    }
                    ,
                    captionType: 'text',
    ...
    // counters
    ( () => {
        const counterElements = document.querySelectorAll('.mod_article.counters');
        if (!counterElements || !counterElements.length) {
            return;
        }
        ...
        const observer = new IntersectionObserver( (entries, observer) => {
            ...
            entries.forEach( (entry) => {
                ...
                if (entry.isIntersecting) {
                    ...
                    const counters = counterElement.querySelectorAll('.content-text:not(:first-of-type)');
                    ...
                    counters.forEach( (counter) => {
                        ...
                        const numberElement = counter.querySelector('.content > p:first-of-type');
                        if (!numberElement || !numberElement.innerHTML) {
                            return;
                        }

                        const endNumber = parseInt(numberElement.innerHTML);
                        let currentNumber = 0;
                        ...
                        const interval = setInterval( () => {
                            ...
                            if (currentNumber >= endNumber) {
                                // end number reached
                                numberElement.innerHTML = endNumber;
