...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'ContaoLoginConfig.php';
...
class FirewallConfig 
{
    ...
    public function contaoLogin(array $value = []): \Symfony\Config\Security\FirewallConfig\ContaoLoginConfig
    {
        if (null === $this->contaoLogin) {
            $this->_usedProperties['contaoLogin'] = true;
            $this->contaoLogin = new \Symfony\Config\Security\FirewallConfig\ContaoLoginConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "contaoLogin()" has already been initialized. You cannot pass values the second time you call contaoLogin().');
        }

        return $this->contaoLogin;
    }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('contao_login', $value)) {
            $this->_usedProperties['contaoLogin'] = true;
            $this->contaoLogin = new \Symfony\Config\Security\FirewallConfig\ContaoLoginConfig($value['contao_login']);
            unset($value['contao_login']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['contaoLogin'])) {
            $output['contao_login'] = $this->contaoLogin->toArray();
        }
