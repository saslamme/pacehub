...
$GLOBALS['TL_DCA']['tl_comments'] = array(
    ...
    // List
    'list' => array('sorting' => array('mode' => \Contao\DataContainer::MODE_SORTABLE, 'fields' => array('date'), 'panelLayout' => 'filter;sort,search,limit'), 'label' => array('fields' => array('name'), 'format' => '%s', 'label_callback' => array('tl_comments', 'listComments')), 'global_operations' => array('all' => array('href' => 'act=select', 'class' => 'header_edit_all', 'attributes' => 'onclick="Backend.getScrollOffset()" accesskey="e"')), 'operations' => array('edit' => array('href' => 'act=edit', 'icon' => 'edit.svg', 'button_callback' => array('tl_comments', 'editComment')), 'delete' => array('href' => 'act=delete', 'icon' => 'delete.svg', 'attributes' => 'onclick="if(!confirm(\'' . ($GLOBALS['TL_LANG']['MSC']['deleteConfirm'] ?? \null) . '\'))return false;Backend.getScrollOffset()"', 'button_callback' => array('tl_comments', 'deleteComment')), 'toggle' => array('href' => 'act=toggle&amp;field=published', 'icon' => 'visible.svg', 'button_callback' => array('tl_comments', 'toggleIcon')), 'show' => array('href' => 'act=show', 'icon' => 'show.svg'))),
    // Palettes
    ...
    // Fields
    'fields' => array('id' => array('sql' => "int(10) unsigned NOT NULL auto_increment"), 'tstamp' => array('sql' => "int(10) unsigned NOT NULL default 0"), 'source' => array('filter' => \true, 'sorting' => \true, 'reference' => &$GLOBALS['TL_LANG']['tl_comments'], 'sql' => "varchar(32) NOT NULL default ''"), 'parent' => array('filter' => \true, 'sorting' => \true, 'sql' => "int(10) unsigned NOT NULL default 0"), 'date' => array('sorting' => \true, 'filter' => \true, 'flag' => \Contao\DataContainer::SORT_MONTH_DESC, 'eval' => array('rgxp' => 'datim'), 'sql' => "varchar(64) NOT NULL default ''"), 'name' => array('exclude' => \true, 'search' => \true, 'inputType' => 'text', 'eval' => array('mandatory' => \true, 'maxlength' => 64, 'tl_class' => 'w50'), 'sql' => "varchar(64) NOT NULL default ''"), 'email' => array('exclude' => \true, 'search' => \true, 'inputType' => 'text', 'eval' => array('mandatory' => \true, 'maxlength' => 255, 'rgxp' => 'email', 'decodeEntities' => \true, 'tl_class' => 'w50'), 'sql' => "varchar(255) NOT NULL default ''"), 'website' => array('exclude' => \true, 'search' => \true, 'inputType' => 'text', 'eval' => array('maxlength' => 128, 'rgxp' => \Contao\CoreBundle\EventListener\Widget\HttpUrlListener::RGXP_NAME, 'decodeEntities' => \true, 'tl_class' => 'w50'), 'sql' => "varchar(128) NOT NULL default ''"), 'member' => array('exclude' => \true, 'inputType' => 'select', 'foreignKey' => 'tl_member.CONCAT(firstname," ",lastname)', 'eval' => array('chosen' => \true, 'doNotCopy' => \true, 'includeBlankOption' => \true, 'tl_class' => 'w50'), 'sql' => "int(10) unsigned NOT NULL default 0", 'relation' => array('type' => 'belongsTo', 'load' => 'lazy')), 'comment' => array('exclude' => \true, 'search' => \true, 'inputType' => 'textarea', 'eval' => array('mandatory' => \true, 'rte' => 'tinyMCE'), 'sql' => "text NULL"), 'addReply' => array('exclude' => \true, 'filter' => \true, 'inputType' => 'checkbox', 'eval' => array('submitOnChange' => \true), 'sql' => "char(1) NOT NULL default ''"), 'author' => array('default' => \Contao\BackendUser::getInstance()->id, 'exclude' => \true, 'inputType' => 'select', 'foreignKey' => 'tl_user.name', 'eval' => array('mandatory' => \true, 'chosen' => \true, 'doNotCopy' => \true, 'includeBlankOption' => \true, 'tl_class' => 'w50'), 'sql' => "int(10) unsigned NOT NULL default 0", 'relation' => array('type' => 'belongsTo', 'load' => 'lazy')), 'reply' => array('exclude' => \true, 'search' => \true, 'inputType' => 'textarea', 'eval' => array('rte' => 'tinyMCE', 'tl_class' => 'clr'), 'sql' => "text NULL"), 'published' => array('exclude' => \true, 'toggle' => \true, 'filter' => \true, 'inputType' => 'checkbox', 'eval' => array('doNotCopy' => \true), 'save_callback' => array(array('tl_comments', 'sendNotifications')), 'sql' => "char(1) NOT NULL default ''"), 'ip' => array('sql' => "varchar(64) NOT NULL default ''"), 'notified' => array('sql' => "char(1) NOT NULL default ''"), 'notifiedReply' => array('sql' => "char(1) NOT NULL default ''")),
);
/**
...
class tl_comments extends \Contao\Backend
{
    ...
    public function checkPermission()
    {
        switch (\Contao\Input::get('act')) {
            ...
            case 'delete':
            case 'toggle':
                $objComment = $this->Database->prepare("SELECT id, parent, source FROM tl_comments WHERE id=?")->limit(1)->execute(\Contao\Input::get('id'));
    ...
    protected function isAllowedToEditComment($intParent, $strSource)
    {
        ...
        switch ($strSource) {
            case 'tl_content':
                $objPage = $this->Database->prepare("SELECT * FROM tl_page WHERE id=(SELECT pid FROM tl_article WHERE id=(SELECT pid FROM tl_content WHERE id=?))")->limit(1)->execute($intParent);
                ...
                break;
            case 'tl_page':
                $objPage = $this->Database->prepare("SELECT * FROM tl_page WHERE id=?")->limit(1)->execute($intParent);
                // Do not check whether the page is mounted (see #5174)
                ...
                break;
            case 'tl_news':
                $objArchive = $this->Database->prepare("SELECT pid FROM tl_news WHERE id=?")->limit(1)->execute($intParent);
            ...
            case 'tl_calendar_events':
                $objCalendar = $this->Database->prepare("SELECT pid FROM tl_calendar_events WHERE id=?")->limit(1)->execute($intParent);
            ...
            default:
                ...
                if (isset($GLOBALS['TL_HOOKS']['isAllowedToEditComment']) && \is_array($GLOBALS['TL_HOOKS']['isAllowedToEditComment'])) {
                    foreach ($GLOBALS['TL_HOOKS']['isAllowedToEditComment'] as $callback) {
                        ...
                        if ($this->{$callback[0]}->{$callback[1]}($intParent, $strSource) === \true) {
                            $cache[$strKey] = \true;
                            break;
                        }
                    }
        ...
        switch ($arrRow['source']) {
            case 'tl_content':
                ...
                if ($objParent->numRows) {
                    $title .= ' – <a href="' . \Contao\StringUtil::specialcharsUrl($router->generate('contao_backend', array('do' => 'article', 'table' => 'tl_content', 'id' => $objParent->id, 'rt' => \REQUEST_TOKEN))) . '">' . $objParent->title . '</a>';
                }
                break;
            case 'tl_page':
                $objParent = $this->Database->prepare("SELECT id, title FROM tl_page WHERE id=?")->execute($arrRow['parent']);
                if ($objParent->numRows) {
                    $title .= ' – <a href="' . \Contao\StringUtil::specialcharsUrl($router->generate('contao_backend', array('do' => 'page', 'act' => 'edit', 'id' => $objParent->id, 'rt' => \REQUEST_TOKEN))) . '">' . $objParent->title . '</a>';
                }
                break;
            case 'tl_news':
                $objParent = $this->Database->prepare("SELECT id, headline FROM tl_news WHERE id=?")->execute($arrRow['parent']);
                if ($objParent->numRows) {
                    $title .= ' – <a href="' . \Contao\StringUtil::specialcharsUrl($router->generate('contao_backend', array('do' => 'news', 'table' => 'tl_news', 'act' => 'edit', 'id' => $objParent->id, 'rt' => \REQUEST_TOKEN))) . '">' . $objParent->headline . '</a>';
                }
                break;
            case 'tl_faq':
                $objParent = $this->Database->prepare("SELECT id, question FROM tl_faq WHERE id=?")->execute($arrRow['parent']);
                if ($objParent->numRows) {
                    $title .= ' – <a href="' . \Contao\StringUtil::specialcharsUrl($router->generate('contao_backend', array('do' => 'faq', 'table' => 'tl_faq', 'act' => 'edit', 'id' => $objParent->id, 'rt' => \REQUEST_TOKEN))) . '">' . $objParent->question . '</a>';
                }
                break;
            case 'tl_calendar_events':
                $objParent = $this->Database->prepare("SELECT id, title FROM tl_calendar_events WHERE id=?")->execute($arrRow['parent']);
                if ($objParent->numRows) {
                    $title .= ' – <a href="' . \Contao\StringUtil::specialcharsUrl($router->generate('contao_backend', array('do' => 'calendar', 'table' => 'tl_calendar_events', 'act' => 'edit', 'id' => $objParent->id, 'rt' => \REQUEST_TOKEN))) . '">' . $objParent->title . '</a>';
                }
                break;
            default:
                // HOOK: support custom modules
