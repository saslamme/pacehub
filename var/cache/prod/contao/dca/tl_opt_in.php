...
$GLOBALS['TL_DCA']['tl_opt_in'] = array(
    // Config
    'config' => array('dataContainer' => \Contao\DC_Table::class, 'closed' => \true, 'notEditable' => \true, 'notCopyable' => \true, 'notDeletable' => \true, 'onshow_callback' => array(array('tl_opt_in', 'showRelatedRecords')), 'sql' => array('keys' => array('id' => 'primary', 'token' => 'unique', 'removeOn' => 'index'))),
    // List
    'list' => array('sorting' => array('mode' => \Contao\DataContainer::MODE_SORTABLE, 'fields' => array('createdOn DESC'), 'panelLayout' => 'filter;sort,search,limit'), 'label' => array('fields' => array('token', 'email', 'createdOn', 'confirmedOn', 'removeOn'), 'showColumns' => \true), 'operations' => array('resend' => array('href' => 'key=resend', 'icon' => 'resend.svg', 'button_callback' => array('tl_opt_in', 'resendButton')), 'show' => array('href' => 'act=show', 'icon' => 'show.svg'))),
    // Fields
...
class tl_opt_in extends \Contao\Backend
{
    ...
    public function showRelatedRecords($data, $row)
    {
        ...
        $objRelated = $this->Database->prepare("SELECT * FROM tl_opt_in_related WHERE pid=?")->execute($row['id']);
