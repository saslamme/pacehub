(function () {
  ...
  function attachErgometer(root){
    var distInput = qs(root, '[data-field="ergo_distance"]');
    var distSlider = qs(root, '[data-field="ergo_distance_slider"]');
    var hInput = qs(root, '[data-field="ergo_time_h"]');
    var mInput = qs(root, '[data-field="ergo_time_m"]');
    var sInput = qs(root, '[data-field="ergo_time_s"]');
    var paceInput = qs(root, '[data-field="ergo_split"]');
    var wattsInput = qs(root, '[data-field="ergo_watts"]');
    var resetBtn = qs(root, '[data-action="reset"]');

    var resDistance = qs(root, '.js-ergo-distance');
    var resTime = qs(root, '.js-ergo-time');
    var resPace = qs(root, '.js-ergo-pace');
    var resSpeed = qs(root, '.js-ergo-speed');
    var resWatts = qs(root, '.js-ergo-watts');
    ...
    var defaultPaceSec = toSec('1:55');
    if (!isFinite(defaultPaceSec) || defaultPaceSec <= 0){
      defaultPaceSec = 115;
    }
    ...
    function paceToWatts(paceSec){
      if (!isFinite(paceSec) || paceSec <= 0) return NaN;
      var perMeter = paceSec / 500;
      if (perMeter <= 0) return NaN;
      return 2.8 / Math.pow(perMeter, 3);
    }

    function wattsToPace(watts){
      if (!isFinite(watts) || watts <= 0) return NaN;
      var perMeter = Math.pow(2.8 / watts, 1 / 3);
      if (!isFinite(perMeter) || perMeter <= 0) return NaN;
      return perMeter * 500;
    }

    function timeFromPaceDistance(paceSec, distance){
      if (!isFinite(paceSec) || paceSec <= 0) return NaN;
      if (!isFinite(distance) || distance <= 0) return NaN;
      return paceSec * (distance / 500);
    }

    function paceFromTimeDistance(timeSec, distance){
      if (!isFinite(timeSec) || timeSec <= 0) return NaN;
      if (!isFinite(distance) || distance <= 0) return NaN;
      return timeSec / (distance / 500);
    }

    function distanceFromPaceTime(paceSec, timeSec){
      if (!isFinite(paceSec) || paceSec <= 0) return NaN;
      if (!isFinite(timeSec) || timeSec <= 0) return NaN;
      return (timeSec / paceSec) * 500;
    }

    function toHoursMinutesSeconds(totalSeconds){
      if (!isFinite(totalSeconds) || totalSeconds <= 0) return { h: '', m: '', s: '' };
    ...
    function formatDistance(m){
      ...
      var base = String(rounded).replace(/\B(?=(\d{3})+(?!\d))/g, '.');
      var text = base + ' m';
      if (rounded >= 1000){
        var kmText = (rounded / 1000).toFixed(2).replace('.', ',');
        text += ' (' + kmText + ' km)';
      }
      return text;
    }

    function formatSpeed(distance, timeSec, paceSec){
      var speedMps = NaN;
      if (isFinite(distance) && isFinite(timeSec) && timeSec > 0){
        speedMps = distance / timeSec;
      } else if (isFinite(paceSec) && paceSec > 0){
        speedMps = 500 / paceSec;
      }
      if (!isFinite(speedMps) || speedMps <= 0) return '–';
      var speedKmh = speedMps * 3.6;
      return speedMps.toFixed(2).replace('.', ',') + ' m/s (' + speedKmh.toFixed(2).replace('.', ',') + ' km/h)';
    }
    ...
    function applyRanges(){
      ...
      if (!isFinite(baseDistMin) || baseDistMin < 0) baseDistMin = 0;
      if (!isFinite(baseDistMax) || baseDistMax <= 0) baseDistMax = 20000;
      if (!isFinite(baseDistStep) || baseDistStep <= 0) baseDistStep = 50;

      if (distInput){
        distInput.min = String(baseDistMin);
        distInput.max = String(baseDistMax);
        distInput.step = String(baseDistStep);
      }
      if (distSlider){
        distSlider.min = String(baseDistMin);
        distSlider.max = String(baseDistMax);
        distSlider.step = String(baseDistStep);
      }
    }

    function recalc(){
      var distanceVal = parseNum(distInput && distInput.value);
      var timeVal = toSecTriple(hInput && hInput.value, mInput && mInput.value, sInput && sInput.value);
      var paceVal = toSec(paceInput && paceInput.value);
      var wattsVal = parseNum(wattsInput && wattsInput.value);

      if (!isFinite(distanceVal) || distanceVal <= 0) distanceVal = NaN;
      if (!isFinite(timeVal) || timeVal <= 0) timeVal = NaN;
      if (!isFinite(paceVal) || paceVal <= 0) paceVal = NaN;
      if (!isFinite(wattsVal) || wattsVal <= 0) wattsVal = NaN;

      if (lastChanged === 'watts' && isFinite(wattsVal)){
        paceVal = wattsToPace(wattsVal);
      }

      if (lastChanged === 'pace' && isFinite(paceVal)){
        if (isFinite(distanceVal)){
          timeVal = timeFromPaceDistance(paceVal, distanceVal);
        } else if (isFinite(timeVal)){
          distanceVal = distanceFromPaceTime(paceVal, timeVal);
        } else {
          distanceVal = defaultDistance;
          timeVal = timeFromPaceDistance(paceVal, distanceVal);
        }
      } else if (lastChanged === 'time' && isFinite(timeVal)){
        if (isFinite(distanceVal)){
          paceVal = paceFromTimeDistance(timeVal, distanceVal);
        } else if (isFinite(paceVal)){
          distanceVal = distanceFromPaceTime(paceVal, timeVal);
        }
      } else if (lastChanged === 'distance' && isFinite(distanceVal)){
        if (isFinite(paceVal)){
          timeVal = timeFromPaceDistance(paceVal, distanceVal);
        } else if (isFinite(timeVal)){
          paceVal = paceFromTimeDistance(timeVal, distanceVal);
      ...
      if (!isFinite(paceVal) && isFinite(timeVal) && isFinite(distanceVal)){
        paceVal = paceFromTimeDistance(timeVal, distanceVal);
      }

      if (!isFinite(distanceVal) && isFinite(timeVal) && isFinite(paceVal)){
        distanceVal = distanceFromPaceTime(paceVal, timeVal);
      }

      if (!isFinite(timeVal) && isFinite(distanceVal) && isFinite(paceVal)){
        timeVal = timeFromPaceDistance(paceVal, distanceVal);
      }

      if (!isFinite(paceVal) && isFinite(wattsVal)){
        paceVal = wattsToPace(wattsVal);
      }

      if (!isFinite(wattsVal) && isFinite(paceVal)){
        wattsVal = paceToWatts(paceVal);
      }

      if (!isFinite(distanceVal) && isFinite(paceVal)){
        distanceVal = defaultDistance;
        timeVal = timeFromPaceDistance(paceVal, distanceVal);
      }

      if (!isFinite(distanceVal) && isFinite(timeVal)){
        paceVal = paceFromTimeDistance(timeVal, defaultDistance);
        distanceVal = defaultDistance;
      }

      if (!isFinite(timeVal) && !isFinite(paceVal) && !isFinite(wattsVal)){
        timeVal = NaN;
      }

      var hasValidCombo = (isFinite(distanceVal) && isFinite(timeVal))
        || (isFinite(distanceVal) && isFinite(paceVal))
        || (isFinite(paceVal) && isFinite(wattsVal));

      if (!hasValidCombo){
        setError(root, 'Bitte Distanz sowie Zeit, Split oder Leistung eingeben.');
      } else {
        setError(root, null);
      }

      if (distInput){
        distInput.value = isFinite(distanceVal) ? String(Math.round(distanceVal)) : '';
      }
      if (distSlider){
        distSlider.value = isFinite(distanceVal) ? String(Math.round(distanceVal)) : distSlider.min || '0';
      }

      if (paceInput){
        if (isFinite(paceVal)){
          paceInput.value = secToMMSS(paceVal);
        } else if (paceInput.value){
          // keep user input if invalid to allow correction
        } else {
          paceInput.value = '';
        }
      }
      ...
      if (wattsInput){
        wattsInput.value = isFinite(wattsVal) ? String(Math.round(wattsVal)) : '';
      }

      if (isFinite(timeVal)){
        var t = toHoursMinutesSeconds(timeVal);
        if (hInput && lastChanged !== 'time') hInput.value = t.h;
        if (mInput && lastChanged !== 'time') mInput.value = t.m;
        if (sInput && lastChanged !== 'time') sInput.value = t.s;
      ...
      if (resDistance) resDistance.textContent = isFinite(distanceVal) ? formatDistance(distanceVal) : '–';
      if (resTime) resTime.textContent = isFinite(timeVal) ? secToHHMMSS(timeVal) : '–';
      if (resPace) resPace.textContent = isFinite(paceVal) ? secToMMSS(paceVal) + ' min/500 m' : '–';
      if (resSpeed) resSpeed.textContent = formatSpeed(distanceVal, timeVal, paceVal);
      if (resWatts) resWatts.textContent = isFinite(wattsVal) ? Math.round(wattsVal) + ' W' : '–';
    }

    if (distInput){
      bindMany(distInput, 'input change', function(){
        lastChanged = 'distance';
        if (distSlider) distSlider.value = distInput.value;
        recalc();
      });
    }
    ...
    [hInput, mInput, sInput].forEach(function(inp){
      if (!inp) return;
      bindMany(inp, 'input change', function(){
        lastChanged = 'time';
        recalc();
      });
    });
    ...
    if (paceInput){
      bindMany(paceInput, 'input change', function(){
        lastChanged = 'pace';
        recalc();
      });
      paceInput.addEventListener('blur', function(){
        if (!paceInput.value) return;
        paceInput.value = normPaceStr(paceInput.value);
        lastChanged = 'pace';
        recalc();
      });
    }
    ...
    if (wattsInput){
      bindMany(wattsInput, 'input change', function(){
        lastChanged = 'watts';
        recalc();
      });
    }
    ...
    qsa(root, '.quick').forEach(function(btn){
      btn.addEventListener('click', function(){
        var d = parseNum(btn.getAttribute('data-distance'));
        if (isFinite(d) && d > 0){
          if (distInput) distInput.value = String(Math.round(d));
          if (distSlider) distSlider.value = String(Math.round(d));
          lastChanged = 'distance';
          recalc();
        }
      });
    ...
    qsa(root, '.preset').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        if (timeParts.length === 3){
          ...
          if (sInput) sInput.value = timeParts[2];
        }
        lastChanged = 'time';
        recalc();
      });
    ...
    if (resetBtn){
      resetBtn.addEventListener('click', function(e){
        e.preventDefault();
        var defaultTime = timeFromPaceDistance(defaultPaceSec, defaultDistance);
        var defaultWatts = paceToWatts(defaultPaceSec);
        if (distInput) distInput.value = String(Math.round(defaultDistance));
        if (distSlider) distSlider.value = String(Math.round(defaultDistance));
        var t = toHoursMinutesSeconds(defaultTime);
        if (hInput) hInput.value = t.h;
        if (mInput) mInput.value = t.m;
        if (sInput) sInput.value = t.s;
        if (paceInput) paceInput.value = secToMMSS(defaultPaceSec);
        if (wattsInput && isFinite(defaultWatts)) wattsInput.value = String(Math.round(defaultWatts));
        lastChanged = null;
        setError(root, null);
        recalc();
      });
    ...
    var initialTime = timeFromPaceDistance(defaultPaceSec, defaultDistance);
    var initialWatts = paceToWatts(defaultPaceSec);
    if (distInput && !distInput.value) distInput.value = String(Math.round(defaultDistance));
    if (distSlider) distSlider.value = String(Math.round(defaultDistance));
    if (paceInput && !paceInput.value) paceInput.value = secToMMSS(defaultPaceSec);
    if (wattsInput && !wattsInput.value && isFinite(initialWatts)) wattsInput.value = String(Math.round(initialWatts));
    if (isFinite(initialTime)){
      var init = toHoursMinutesSeconds(initialTime);
      if (hInput && !hInput.value) hInput.value = init.h;
      if (mInput && !mInput.value) mInput.value = init.m;
      if (sInput && !sInput.value) sInput.value = init.s;
    }
  ...
  function boot(){
    qsa(document, '[data-calculator]').forEach(function(root){
      ...
      if (type === 'ergometer') attachErgometer(root);
